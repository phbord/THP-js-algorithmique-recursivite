...
const computeSquareRoot = (x, n) => {
    if (!x) x = n/2; //4.5
    let div = n/x; //2
    let resDiv = (div + x)/2; //6.5/2 = 3.25
    return res = x === resDiv ? x : computeSquareRoot(n, resDiv);
};
...
const isPrimeNumber = (n) => {
    let squareRoot = parseInt(Math.sqrt(n));
    let divisor = 2;
...
console.log('Nombre premier en récursive: ', isPrimeNumber(9)); //
